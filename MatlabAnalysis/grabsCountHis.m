function grabsHis(outputPath, generalProperty, imagingData, BehaveData)
...
%      X = imagingData.samples(:, :, examinedInds);
     ...
     for trailsHisIndex = 1:length(examinedInds)
         for i = 1:length(grabsEvents)
            sumTrailCurrentEvent = double(sum(BehaveData.(grabsEvents{i}).indicator(examinedInds(trailsHisIndex), t >= toneTime & t<= toneTime + 2)) > 0);
%             sumTrailCurrentEvent = BehaveData.(grabsEvents{i}).indicatorPerTrial(examinedInds(trailsHisIndex));
...
%      mysave(fig2, fullfile(outputPath, 'SF_grabs_trials_average'));
    ...
    fileID = fopen([outputPath, '\ttest.txt'],'w');
