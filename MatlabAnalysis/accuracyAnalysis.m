...
figure;
myplotConfMat(confMatsPrev(:,:,findClosestDouble(time4confplotNext, tmid)), eventsList);
title(['Cont mat @ ' num2str(time4confplotNext) 'secs']);
...
figure;
myplotConfMat(confMats(:,:,findClosestDouble(time4confplot, tmid)), eventsList);
title(['Cont mat @ ' num2str(time4confplot) 'secs']);
mysave(gcf, fullfile(outputPath, ['conf_' foldstr linstr eventsStr]));
