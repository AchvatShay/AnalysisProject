        function doGlmAnalysisHistWithoutParalell(BehaveData, timesegments, t, ttraj, foldsNum, INDICES, imagingData, ...
    ...
    for time_seg_i = 1:size(timesegments,2)
        ...
        for nrni = 1:size(imagingData.samples, 1)           
            ...
            for fold_i = 1:foldsNum
               ...
               [x, x0, R2Tr, R2Te, fold_in, nrni, ~] = LassoFunction(x_train{fold_i}, Ytr(:), foldsNum, x_test{fold_i}, Yte(:), fold_i, nrni, -1, distribution, linkFunction);
        ...
        for nrni = 1:size(imagingData.samples, 1)
            ...
            for fold_i = 1:foldsNum
                ...
                for type_i = 1:length(types)
                    ...
                    [x_c, x0_c, R2Tr_cur, R2Te_cur, fold_in_c, cur_nrni, type_in] = LassoFunction(x_train{fold_i}(:, binds), Ytr(:), foldsNum, x_test{fold_i}(:, binds), Yte(:), fold_i, nrni, type_i, distribution, linkFunction);
