function [SpikeTrainStartF, SpikeTrainEndF, SpikeTrainPksF,SpikeTrainHF, eventDetector_activityF, comb_activityF, fitData] = calcRoiEventDetectorByMLSpike_V3(dataCurROI, ImageSampleTime, frameNum, aV, outputpath, activityIndex, clusterCount, roiName, sigmaChangeValue, roiCount, runMLS, thresholdForGn)          
    sampFreq        = 1/ImageSampleTime;
 
    if runMLS
        for i = 1:size(dataCurROI,2)
            traceSigAll(i) = {squeeze(dataCurROI(:,i))};
        end 
        ...
        [spk, fitCell, drift, parest] = spk_est(traceSigAll,par);

    % -------------------------------------------------------------------------------    
        for i = 1:length(spk)
            countVecCell(i) = {fn_timevector(spk{i}, ImageSampleTime, 'count')};
        end
    else
        traceSigAll = dataCurROI;
        spkmin = thresholdForGn*GetSn(traceSigAll);
        model_ar = 'ar2';
        fr = ImageSampleTime;
        decay_time = 0.5;
        lam = choose_lambda(exp(-1/(fr*decay_time)),GetSn(traceSigAll),0.99);
        [cc,spk,opts_oasis] = deconvolveCa(traceSigAll,model_ar,'method','thresholded','optimize_pars',true,'maxIter',20,...
                                    'window',150,'lambda',lam,'smin',spkmin);
        fitCell = {cc};                        
        countVecCell = {spk ~= 0};
    end
    
    for tr_index = 1:length(fitCell)
        traceSig = squeeze(dataCurROI(:,tr_index));
        fit = fitCell{tr_index};
        countVec = countVecCell{tr_index};
...
%     close(f);
    ...
    %         eventDetector_activity((SpikeTrainStart(i)):(SpikeTrainPks(i))) = ones(length((SpikeTrainStart(i)):SpikeTrainPks(i)),1) .* roiCount;
        ...
        SpikeTrainStartF(tr_index) = {SpikeTrainStart};
        SpikeTrainEndF(tr_index) = {SpikeTrainEnd};
        SpikeTrainPksF(tr_index) = {SpikeTrainPks};
        SpikeTrainHF(tr_index) = {SpikeTrainH};
        comb_activityF(tr_index) = {comb_activity};
        eventDetector_activityF(tr_index) = {eventDetector_activity};        
        fitData(tr_index) = {fit};
    end
end